...
const services: { id: LauncherServiceId; title: string; description: string }[] = [
  ...
  { id: "tts", title: "TTS 语音引擎", description: "把文字合成为声音，启动时会加载语音模型。" },
  { id: "windows", title: "Windows 执行端", description: "连接 Windows，负责声音播放、VTS 口型和 OBS 控制。" },
];
...
export function LauncherControls({ controller }: { controller: ReturnType<typeof useLauncher> }) {
  ...
  return <section className="panel launcher-panel" aria-labelledby="launcher-heading">
      ...
      <ol className="launcher-steps">
        ...
        <li><strong>开启 TTS</strong><span>首次加载通常需要几十秒；启动中可关闭开关取消。</span></li>
        <li><strong>连接 Windows 执行端</strong><span>主服务就绪后打开第三个开关，等到“已连接”即可播放声音。</span></li>
        <li><strong>准备自己的音色</strong><span>首次没有预置声音：到“角色与音色”上传参考录音，再到“训练与离线”训练、试听并保存。选择自己的音色后即可试播。</span></li>
      </ol>
      {snapshot && <>
        <p className={snapshot.setup.llm_configured ? "field-hint" : "availability-note"}>{snapshot.setup.llm_message}</p>
        <details className="launcher-help"><summary>首次使用、配置位置与退出方法</summary>
          ...
          <p>默认 VTS / OBS 关闭，先测试声音。后续再按启动手册开启 VTS 与 OBS。</p>
