...
const snapshot = (overrides: Partial<LlmSettingsSnapshot> = {}): LlmSettingsSnapshot => ({
  settings: {
    ...
    model: "gpt-4o-mini", mode: "cloud", timeout_seconds: 30, max_tokens: 1024, json_mode: true,
  },
...
describe("LLM 接入面板", () => {
  it("加载时不回填密钥，测试使用当前草稿且不自动保存", async () => {
    ...
    expect(await screen.findByRole("textbox", { name: "模型名称" })).toHaveValue("gpt-4o-mini");
    ...
    await userEvent.type(screen.getByRole("textbox", { name: "模型名称" }), "gpt-4.1-mini");
  ...
  it("切换供应商套用协议和地址、清空模型，并要求明确处理密钥", async () => {
    ...
    await userEvent.type(screen.getByLabelText("API 密钥"), "temporary");
    await userEvent.selectOptions(screen.getByRole("combobox", { name: "服务商" }), "gemini");
    expect(screen.getByLabelText("API 密钥")).toHaveValue("");

    await userEvent.type(screen.getByRole("textbox", { name: "模型名称" }), "gemini-2.5-flash");
    ...
    expect(await screen.findByText(/重启主服务/)).toBeVisible();
  ...
  it("忽略已取消加载的迟到响应，不覆盖新 client 的草稿", async () => {
    ...
    expect(await screen.findByDisplayValue("current-model")).toBeVisible();
    await act(async () => late.resolve(snapshot({ settings: { ...snapshot().settings, model: "stale-model" } })));
    expect(screen.getByRole("textbox", { name: "模型名称" })).toHaveValue("current-model");
  });
  ...
  it("切换 client 会取消旧操作并忽略迟到的测试结果", async () => {
    ...
    await userEvent.click(screen.getByRole("button", { name: "测试连接" }));
    view.rerender(<LlmPanel client={second} />);
    expect(await screen.findByDisplayValue("new-service-model")).toBeVisible();
  ...
  it("加载失败可重试，卸载取消请求，操作期间禁用控件", async () => {
    ...
    expect(await screen.findByDisplayValue("gpt-4o-mini")).toBeVisible();
  });
});
