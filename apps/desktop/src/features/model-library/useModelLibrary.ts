    ...
    try {
      ...
      if (mounted.current && !controller.signal.aborted) {
        ...
        if (download?.state === "failed") {
          ...
          message: kind === "select" && next.installed.find(item => item.id === id)?.purpose === "asr" ? "语音识别模型已选择，下一次提取文本或仅语音训练时生效。" : kind === "download" ? "请在下载列表查看进度和最终结果；模型可用性以扫描结果为准。" : kind === "select" ? "模型已切换，TTS 正在自动启动，请等待运行状态就绪。" : kind === "scan" ? `已扫描模型目录，发现 ${next.installed.length} 个模型。` : "请查看下载列表确认最新状态。" });
      }
    } catch (failure) {
