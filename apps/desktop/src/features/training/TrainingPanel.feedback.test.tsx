...
it.each([true, false])("HTTP 成功也根据 verified=%s 判断测量通过与否", async verified => {
  ...
  const dialog = await screen.findByRole("dialog", { name: verified ? "离线测量成功" : "离线测量失败" });
  expect(within(dialog).getByText(verified ? /本次测量通过/ : /显存峰值超过限制/)).toBeVisible();
});
it("提交只提示任务已接收，不把排队当作训练完成", async () => {
  ...
  const dialog = await screen.findByRole("dialog", { name: "训练已提交" });
  expect(within(dialog).getByText(/训练记录/)).toBeVisible();
...
it.each(["completed", "failed"])("后台任务转为 %s 时提示一次，历史终态与重复轮询不弹", async status => {
  ...
  const dialog = screen.getByRole("dialog", { name: status === "completed" ? "训练完成" : "训练失败" });
  expect(dialog).toHaveTextContent("我的音色");
  if (status === "failed") expect(dialog).toHaveTextContent("降低");
