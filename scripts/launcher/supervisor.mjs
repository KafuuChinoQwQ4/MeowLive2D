...
export class Supervisor {
  ...
  async snapshot() {
    ...
    return { schema_version: 1, session_token: this.token, setup: this.setup,
      services: [...this.records.values()].map(record => ({ id: record.def.id, state: record.state,
        ...
        can_start: !this.closed && !record.def.issue && !record.child && !record.occupied,
        can_stop: Boolean(record.child) && record.state !== 'stopping' })) };
  }
  ...
  async configureTts(update) {
    const record = this.records.get('tts');
    return this.locked(record, async () => {
      if (this.closed) throw new Error('控制面板正在退出。');
      const health = await inspectEndpoint(record.def);
      if (record.child || health.occupied || record.state === 'stopping') throw new Error('请先关闭 TTS，再切换语音模型；外部服务请回原终端关闭。');
      record.def = await update(record.def);
      record.state = 'stopped'; record.occupied = false;
      record.message = record.def.issue ?? '模型已选择，可打开 TTS 开关。';
    });
  }
