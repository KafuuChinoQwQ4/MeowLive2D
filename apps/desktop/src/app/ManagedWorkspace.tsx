...
export function ManagedWorkspace({ pages }: Pick<WorkspaceProps, "pages">) {
  ...
  return <Workspace pages={pages} setup={<ModelLibraryPanel client={modelClient} token={controller.stale ? null : controller.snapshot?.session_token ?? null} onSelected={controller.refresh} />} overview={<LauncherControls controller={controller} />} ready={available("server")}
    status={(["server", "tts", "windows"] as const).map(id => ({ label: `${titles[id]} · ${label(id)}`, available: controller.stale ? null : available(id) }))}
    notice={!available("tts") && <p className="availability-note">TTS 尚未就绪，可以先配置角色；播报前请在“启动与运行”打开 TTS 开关。</p>} />;
}
